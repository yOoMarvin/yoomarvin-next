...
import type { PostMeta, Post, PageObjectResponse, BlockObjectResponse } from './types'
...
function pageToMeta(page: PageObjectResponse): PostMeta {
  ...
  return {
    ...
    slug: props.Slug?.type === 'rich_text' ? props.Slug.rich_text[0]?.plain_text ?? '' : '',
    status: props.Status?.type === 'select' ? (props.Status.select?.name as PostMeta['status']) ?? 'Draft' : 'Draft',
    date: props.Date?.type === 'date' ? props.Date.date?.start ?? null : null,
    ...
    if (block.type === 'bulleted_list_item') {
      ...
      elements.push(
        <ul key={items[0].id}>
          {items.map((item) => (
            <li key={item.id}>
              {renderRichText((item as Record<string, unknown>).bulleted_list_item.rich_text)}
            </li>
          ))}
        </ul>
      )
    ...
    if (block.type === 'numbered_list_item') {
      ...
      elements.push(
        <ol key={items[0].id}>
          {items.map((item) => (
            <li key={item.id}>
              {renderRichText((item as Record<string, unknown>).numbered_list_item.rich_text)}
            </li>
          ))}
        </ol>
      )
...
function renderRichText(richText: RichTextItemResponse[]) {
  ...
  return richText.map((t, i) => {
    ...
    if (t.href) node = <a key={i} href={t.href}>{node}</a>

    const hasAnnotation = t.annotations.bold || t.annotations.italic ||
      t.annotations.strikethrough || t.annotations.underline ||
      t.annotations.code || t.href
