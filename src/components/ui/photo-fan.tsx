    ...
    const handleTouchStart = useCallback((e: React.TouchEvent) => {
        ...
        if (closestIndex !== -1) {
            setHoveredIndex((prev) =>
                prev === closestIndex ? null : closestIndex
            )
        }
    }, [])
        ...
        return (
            <div className="flex justify-center">
                {CARDS.map(({ src, label }, i) => (
                    <div
                        key={i}
                        className={`relative aspect-[3/4] w-30 md:w-36 shrink-0 rounded-2xl overflow-hidden bg-zinc-200 dark:bg-zinc-700 ring-1 ring-inset ring-black/10 dark:ring-white/10 ${i > 0 ? '-ml-16' : ''}`}
                    >
                        <Image
                            src={src}
                            alt={label}
                            ...
                            sizes="(max-width: 768px) 120px, 144px"
                        />
                        <p className="absolute bottom-2.5 left-3 right-3 font-mono text-xs text-white text-balance">
                            {label}
                        </p>
                    </div>
                ))}
            ...
            {CARDS.map(({ src, label }, i) => {
                ...
                const deflection =
                    hoveredIndex !== null && !isHovered
                        ? (16 / Math.max(1, Math.abs(distance) * 0.6)) *
                          Math.sign(distance)
                        : 0
                return (
                    <PhotoCard
